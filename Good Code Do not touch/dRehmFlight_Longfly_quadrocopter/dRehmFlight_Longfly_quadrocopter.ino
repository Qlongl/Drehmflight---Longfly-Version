//Longfly quadrocopter code prototype:
// list of changes:
// implementation of a "Quick_Update_PID_COEFF" function for quick pid gain testing
// implementation of print_dynamic_PID_coeff , useful for troubleshooting PID gains
// set to rate mode, 160 degrees rotation

//Issues with code:
// motors get very hot
// arms when radio is not connected
// does not disarm when disarm switch flicked
// slight drift in flight (probably due to not allowing gyro to cascade to correct value)
...
float GyroErrorX = -10.50;
float GyroErrorY = -2.06;
float GyroErrorZ = -1.78;


//Controller parameters (take note of defaults before modifying!): 
float i_limit = 25.0;     //Integrator saturation level, mostly for safety (default 25.0)
float maxRoll =  160.0;    //Max roll angle in degrees for angle mode (maximum ~70 degrees), deg/sec for rate mode 
float maxPitch = 160.0;   //Max pitch angle in degrees for angle mode (maximum ~70 degrees), deg/sec for rate mode
float maxYaw = 160.0;     //Max yaw rate in deg/sec

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

//////////////////          HERE ARE WHERE THE PID COEFFICIENTS ARE DEFINED, MODIFY THESE VALUES TO PID TUNE YOUR DRONE         ////////////////////////

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

// angle mode is for beginner flying, automatically stabalizes drones to fly around a given angle, same as angle mode in betaflight
// rate mode is for acrobatic flying, stabalizes the drone to fly around a given roll rate, same as acro / air mode in betaflight
...
float Ki_roll_rate = 0.0;     //Roll I-gain - rate mode
float Kd_roll_rate = 0.0;     //Roll D-gain - rate mode (be careful when increasing too high, motors will begin to overheat!)

float Kp_pitch_rate = 0.05;    //Pitch P-gain - rate mode
float Ki_pitch_rate = 0.0125;    //Pitch I-gain - rate mode
float Kd_pitch_rate = 0.0005;    //Pitch D-gain - rate mode (be careful when increasing too high, motors will begin to overheat!)

float Kp_yaw = 0.7;           //Yaw P-gain
float Ki_yaw = 0.05;          //Yaw I-gain
float Kd_yaw = 0.000;         //Yaw D-gain (be careful when increasing too high, motors will begin to overheat!)
...
{
  int result = sscanf(txt, "%f %f %f", &Kp_pitch_rate, &Ki_pitch_rate, &Kd_pitch_rate);
  
  if (result == 3) {
    Serial.println("Updated Pitch PID coefficients:");
    Serial.print("P: ");
    Serial.println(Kp_pitch_rate);
    Serial.print("I: ");
    Serial.println(Ki_pitch_rate);
    Serial.print("D: ");
    Serial.println(Kd_pitch_rate);
  } else {
...
//qlong4
  ...
  m1_command_scaled = thro_des + pitch_PID + roll_PID + yaw_PID; // bottom left
  m2_command_scaled = thro_des - pitch_PID + roll_PID - yaw_PID; // top left
  m3_command_scaled = thro_des + pitch_PID - roll_PID - yaw_PID; // bottom right
  m4_command_scaled = thro_des - pitch_PID - roll_PID + yaw_PID; // top righ
  m5_command_scaled = 0;
  m6_command_scaled = 0;

  //0.5 is centered servo, 0.0 is zero throttle if connecting to ESC for conventional PWM, 1.0 is max throttle
  s1_command_scaled = channel_6_pwm;
  s2_command_scaled = 0;
  s3_command_scaled = 0;
  s4_command_scaled = 0;
  s5_command_scaled = 0;
...
roll_des => constrain -1,1

Theory: PID loop is based off of a GyroX = 1 value. However GyroX = 1 might be equal to 5
...
*/
  ...
  integral_pitch = constrain(integral_pitch, -i_limit, i_limit); //Saturate integrator to prevent unsafe buildup
  derivative_pitch = (error_pitch - error_pitch_prev)/dt; 
  pitch_PID = .06*(Kp_pitch_rate*error_pitch + Ki_pitch_rate*integral_pitch + Kd_pitch_rate*derivative_pitch); //Scaled by .01 to bring within -1 to 1 range

  //Yaw, stablize on rate from GyroZ
// this is yaw rate, qlong9
