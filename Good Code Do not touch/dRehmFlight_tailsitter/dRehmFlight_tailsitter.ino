// fully up and working code
// list of qlong changes, 
// armed fly now set to fly if pwm_5 >1500, not <1500, 
// this is so that on instance of no pulses it dissarms instead of goes to max throttle
...
float B_madgwick = 0.04;  //Madgwick filter parameter // used to be 0.04 // note, this is limted to be between 0 and 1
...
// qlong1 => calibrated imu
// looks like the source of the 26* error is a combination of poor imu calibration & B_madgwick too low. 
// possible as well that on a flat surface the pitch position remains constant, but on non-flat surface it 
//starts to accumulate error

//IMU calibration parameters - calibrate IMU using calculate_IMU_error() in the void setup() to get these values, then comment out calculate_IMU_error()
// comented out one is the home build, current one is library free standing

// float AccErrorX = 0.06;
// float AccErrorY = 0.02;
// float AccErrorZ = -0.14;
// float GyroErrorX = -1.55;
// float GyroErrorY = 0.28;
// float GyroErrorZ = 0.57;


// these were recorded with the teensy parallel to the ground
float AccErrorX = 0.07;
float AccErrorY = 0.02;
float AccErrorZ = -0.10;
float GyroErrorX = -1.96;
float GyroErrorY = 0.76;
float GyroErrorZ = 1.31;
...
float maxPitch = 70.0;    //Max pitch angle in degrees for angle mode (maximum ~70 degrees), deg/sec for rate mode
float maxYaw = 220.0;     //Max yaw rate in deg/sec

float Kp_roll_angle = 0.2;    //Roll P-gain - angle mode // was 0.0

//qlong6 finds another set of PID gains to modify???
//float Ki_roll_angle = 0.0;    //Roll I-gain - angle mode // was previously 0.3

float Kd_roll_angle = 0;//0.05;   //Roll D-gain - angle mode (has no effect on controlANGLE2)
...
//Radio communication:
unsigned long channel_1_pwm, channel_2_pwm, channel_3_pwm, channel_4_pwm, channel_5_pwm, channel_6_pwm, channnel_7_pwm;
...
// control gains
// hover
float kp_pitch_angle_hover = 1.25;
float Ki_pitch_angle = 1.0;
float kd_pitch_angle_hover = 0.4;




float kp_roll_angle_hover = 0.75;
float Ki_roll_angle = 0.1; 
float kd_roll_angle_hover = -0.35;




float kp_yaw_hover = 0.46;
...
// servo trims
float servo_right_trim = 0.5;
float servo_left_trim = 0.5;
...
//Quick_update_PID_COEFF txt string
char txt[BUFFER];
int i=0;
...
void loop() {
  ...
  //print_armed_status(); //prints armed status // qlong_diagnosis


  //printKd_roll_angle(); // used to diagnose which part of the roll PID's is not becoming zero dispite my clear instructions

  //print_PID_dynamic(); // used to diagnose the individual weights of PID gains oh my god so good
 
  ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

  ////////////////// UNCOMMENT Quick_Update_PID_COEFF() WHILE PID TUNING! IT MAKES PID TUNING WAYYYYYY FASTER!!! ////////////////
  
  ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////



  // Quick_Update_PID_COEFF();

  // Explanation: This makes it so that you can write PID values without having to re-flash the teensy each time (which takes
  // 10-20 seconds, gets annoying real quick), ('Cmd' + 'F' Quick_update_PID_COEFF to find instructions on how to use 
  // Quick_Update_PID_COEFF 
...
void Quick_Update_PID_COEFF() 

// // The way it works:
// // Quick_Update_PID_COEFF reads user-input values from the serial monitor and passes them to the "parse_txt" function, 
// // which temporarily updates your PID coefficients in the "User-Specified Variables" section of the dRehmflight code.
{
    if (!Serial.available()) {
      return;
    }
    Serial.print("Type your desired PID COEFF as: 'P_value' 'I_value' 'D_Value' then hit return: ");
    char c = Serial.read();
    if (i == BUFFER-1) {
      // overflow! No newline found
      i = 0;
      return;
    }
...
// This allows you to test new PID values quickly without re-uploading the code each time. 
// However, to save these values permanently, you must manually update the kp_angle, ki_angle (for angle mode), 
// or kp_rate (for rate/acro mode) in the "User-Specified Variables" section, otherwise your PID values will be wiped out 
// the next time you upload code to the teensy
}

/********************************************************************************************************************************************************/
////////////////// BY DEFAULT Quick_Update_PID_COEFF only updates the pitch axis PID coeffients!!!                     ///////////////////////////////////
////////////////// CHANGE Kp / Ki / Kd _Pitch_angle to Kp_Roll_angle or Kp_yaw in void "Parse_txt"                     ///////////////////////////////////
////////////////// To have Quick_Update_PID_COEFF write your values to the roll or yaw axis PID values respectively!!! ///////////////////////////////////
/********************************************************************************************************************************************************/

void parse_txt() 
// this function takes the text that Quick_Update_PID_COEFF() recorded and overwrites your PID coefficents,
// Kp_(roll/pitch/yaw), Ki_(roll/pitch/yaw), Kd_(roll/pitch/yaw) respectively, though by default it only 
// overwrites the pitch PID values
{
  int result = sscanf(txt, "%f %f %f", &Kp_roll_angle, &Ki_roll_angle, &Kd_roll_angle);
  
  if (result == 3) {
    Serial.println("Updated Pitch PID coefficients:");
    Serial.print("P: ");
    Serial.println(Kp_roll_angle);
    Serial.print("I: ");
    Serial.println(Ki_roll_angle);
    Serial.print("D: ");
    Serial.println(Kd_roll_angle);
  } else {
    Serial.println("cannot parse text:");
    Serial.println(txt);
  }
}
...
void controlMixer() {
  ...
  //qlong2 qlong_mixer
  // from looking at transition fader, transition_fader = pwm signal for chanel 6: Therefor switch up = hover mode


  // bicopter tailsitter control mixer formating:
...
// uncomment for ff mode yaw testing, include transition fader so gyro is in correct state

  // servos arnt working, I know it must be because of the second part of the code, therefore it must be either the servo trim, yaw PID, or pitch PID
  //0.5 is centered servo, 0.0 is zero throttle if connecting to ESC for conventional PWM, 1.0 is max throttle
  // uncomment for servo actuation
  s2_command_scaled = (1.0 - transition_fader)*(servo_left_trim + roll_PID + pitch_PID  ) + (transition_fader)*(servo_left_trim - yaw_PID + pitch_PID);  // left
  s1_command_scaled = (1.0 - transition_fader)*(servo_right_trim + roll_PID - pitch_PID ) + (transition_fader)*(servo_right_trim - yaw_PID - pitch_PID); // right
...
// qlong3 => this part is sketchy, used to be 
  ...
  pitch_IMU = (1.0 - transition_fader)*(-asin(constrain(-2.0f * (q1*q3 - q0*q2),-0.99999,0.99999))*57.29577951+0.0f) + (transition_fader)*(atan2(q1*q3 - q0*q2, 0.5f - (q1*q1 + q2*q2))*57.29577951+90.0f); // swaped +90.0f of forward flight with hover
...
//qlong9 relevant location of PID_Yaw, PID_roll, ect
void controlANGLE() {
  ...
  //qlong11 check this
  //Pitch
  error_pitch = pitch_des - pitch_IMU; //last change
...
void print_armed_status()
{
  if(armedFly == false)
  {
    Serial.println(F(" armedFly = false"));
  }
  else if(armedFly == true)
  {
    Serial.println(F(" armedFly = true"));
  }
  if(armedFly != true && armedFly != false)
  {
    Serial.println(F("test failed, error in armedFly"));
  }
}

//qlong8 code diagnosis
void printKd_roll_angle()
{
  if (current_time - print_counter > 10000) {
    print_counter = micros();

    Serial.print(F("Kp_yaw: "));
    Serial.print(Kp_yaw);
    Serial.print("\n");
    Serial.print("Ki_yaw:");
    Serial.print(Ki_yaw);
    Serial.print("\n");
    Serial.print(F("Kd_yaw_ff: "));
    Serial.print(Kd_yaw);
    Serial.print("\n");
    ...
    // Serial.print(F("error_pitch: "));
    // Serial.println(error_pitch);
    }
}

void print_PID_dynamic()
{
  if (current_time - print_counter > 10000) {
    print_counter = micros();
    // Serial.print(F(" Printing Roll_PIDS: "));
    // Serial.print(F("P_ component: "));
    // Serial.print(Kp_roll_angle*error_roll);
    // Serial.print(" I_component: ");
    // Serial.print(Ki_roll_angle*integral_roll);
